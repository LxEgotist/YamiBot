...
def saveMessage(message_id: str, message: str, user: str, group: Optional[str] = None) -> None:
    ...
    """
    file_group = Path('.') / 'LxBot' / 'data' / 'data_Group' / f'{group}' / 'message.json'
    file_private = Path('.') / 'LxBot' / 'data' / 'data_Log' / 'message_private.json'
    ...
    if group:
        ...
        try:
            ...
            os.mkdir(Path('.') / 'LxBot' / 'data' / 'data_Group' / f'{group}')
            with open(file_group, 'w') as f:
                f.write(json.dumps(data_group))
                f.close()
    else:
        ...
        try:
            ...
            os.mkdir(Path('.') / 'LxBot' / 'data' / 'data_Log')
            with open(file_private, 'w') as f:
                f.write(json.dumps(data_private))
                f.close()
