'''
Descripttion: 
version: 
Author: LxEgotist
Date: 2020-11-06 13:05:57
LastEditors: LxEgotist
LastEditTime: 2020-11-07 00:09:50
'''
""" 插件数据
"""
import configparser
import os
import pickle
from typing import IO
from pathlib import Path
from nonebot import get_driver

class PluginData:
	""" 插件数据管理
	将插件数据保存在 `data` 文件夹对应的目录下。
	提供保存和读取文件/数据的方法。
	"""
	def __init__(self, name: str, config: bool = False) -> None:
		# 插件名，用来确定插件的文件夹位置
		self._name = name
		self._base_path = Path(".") / "data" / f'plugin-{self._name}'

		# 如果文件夹不存在则自动新建
		os.makedirs(self._base_path, exist_ok=True)

		# 如果需要则初始化并加载配置
		if config:
			self._config_path = Path(".")/self._base_path / f'{self._name}.ini'
			self.config = configparser.ConfigParser()
			if self._config_path.exists():
				self._load_config()
			else:
				self._save_config()

	def save_pkl(self, data: object, filename: str) -> None:
		with self.open(f'{filename}.pkl', 'wb') as f:
			pickle.dump(data, f)

	def load_pkl(self, filename: str) -> object:
		with self.open(f'{filename}.pkl', 'rb') as f:
			data = pickle.load(f)
		return data

	def get_config(self, section: str, option: str, fallback: str = '') -> str:
		""" 获得配置
		如果配置不存在则使用 `fallback` 并保存
		如果不提供 `fallback` 默认返回空字符串
		"""
		try:
			value = self.config.get(section, option)
		except (configparser.NoSectionError, configparser.NoOptionError):
			value = fallback
			# 保存默认配置
			if section not in self.config.sections():
				self.config[section] = {}
			self.config.set(section, option, fallback)
			self._save_config()
		return value

	def set_config(self, section: str, option: str, value: str) -> None:
		""" 设置配置
		"""
		if section not in self.config.sections():
			self.config[section] = {}
		self.config.set(section, option, value)
		self._save_config()

	def _load_config(self) -> None:
		""" 读取配置
		"""
		self.config.read(self._config_path, encoding='utf8')

	def _save_config(self) -> None:
		""" 保存配置
		"""
		with open(self._config_path, 'w', encoding='utf8') as configfile:
			self.config.write(configfile)

	def open(self, filename: str, open_mode: str = 'r') -> IO:
		path = Path(".")/self._base_path / filename
		return open(path, open_mode)

	def exists(self, filename: str) -> bool:
		""" 判断文件是否存在
		"""
		path = Path(".")/self._base_path / filename
		return path.exists()