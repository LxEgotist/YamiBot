...
import uuid
data_path=Path('.')/'LxBot'/'data'/'nokia'
font_size = 70
line_gap = 20
body_pos = (200, 330)
...
font = ImageFont.truetype(str(data_path/'fonts'/'1.ttf'), font_size)
...
def generate_image(text: str,save_path=Path('.')):
    origin_im = Image.open(str(data_path/'images'/'3.png'))
    text = text[:900]
    length = len(text)
    width, height = font.getsize(text)
    current_width = 0
    ...
    save_path=save_path/f"{str(uuid.uuid4()).split('-')[-1]}.png"
    origin_im.save(save_path, "png")
