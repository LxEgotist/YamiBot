...
# @last-modified 2020-11-11T19:29:00.375Z+08:00
...
async def _(bot: Bot, event: Event, state: dict):
	...
	for data in js:
		if data["id"] != "0":
			kwargs={"group_id":data["group"],"message":data["date"]}
			sc.add_job(send_msg,"date",kwargs=kwargs,id=data["uuid"],run_date=datetime.strptime(data['date'],date_format))
	pass
'''
@scheduler.scheduled_job(send_msg,"date",kwargs=kwargs,id=data["uuid"],run_date=datetime.strptime(data['date'],date_format))
async def _():

'''
async def send_msg(group_id:int,message:str):
		...
		if msg[0] == 'help':
			...
			if msg[0] == "add":
				...
				if not re.findall(pattern,date):
					...
					with open(fname,'w') as f:
						json.dump(js,f)
					kwargs={"group_id":event.group_id,"message":data["date"]}
					sc.add_job(send_msg,"date",kwargs=kwargs,id=data["uuid"],run_date=datetime.strptime(data['date'],date_format))
