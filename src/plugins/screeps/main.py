...
###############################
###############################
def screeps_map(shard: str ,roomName: str,data_place=Path('.'))->(int,str):
	resources_pth=data_place
	rawConfig=''
	with open(data_place/'screeps_config.json','r')as f:
		rawConfig=f.read()
		f.close()
	try:
	...
	try:
		...
		if len(roomName)!=6 or (not checkRoomName(roomName)):
			...
			return -1,"参数错误"
	# 命令行参数检查完成,
	ws = create_connection('wss://screeps.com/socket/666/lxegoist/websocket')
	for i in range(4):
	...
	try:
	    ...
	    # Render!
	    res_dir = resources_pth/"imgs"
	    rpname = 'default'
	    pixs = 16
	    bg = Image.open(resources_pth/'resourcepacks' / rpname / 'textures' / 'bg.png').convert("RGBA")
	    rampart_list = []
	        ...
	        if structure == 'extension':
	            ...
	            if obj['store']['energy'] < obj['storeCapacityResource']['energy']/3:
	                structure_image = Image.open(
	                    res_dir / f"{structure}_empty.png").convert("RGBA")
	            else:
	                structure_image = Image.open(
	                    res_dir / f"{structure}.png").convert("RGBA")
	    ...
	    res_dir = resources_pth / 'resourcepacks'/'default' / 'textures'
	...
	ws.close()
	if not os.path.exists(data_place/'data_Temp'/'screeps'):
		os.makedirs(data_place/'data_Temp'/'screeps')
	tmp_img=data_place/ 'data_Temp' /'screeps' /f"screeps-room-{shard}-{roomName}.png"
	bg.save(tmp_img)
	return (1,tmp_img)
if __name__=='__main__':
	try:
		...
		if len(roomName)!=6 or (not checkRoomName(roomName)):
			...
			help(e)
	print(screeps_map(shard,roomName,data_place=Path('.')/'LxBot'/'data'))